...
| `filler.type` | `음` · `어` · `그` · `저` · `뭐` · `뭔가` · `좀` · `막` · `그냥` · `같다` · `네` · `그렇다` · `기타` |
...
- `filler.type` 탐지 방식은 두 갈래로 나뉜다 (2026-09-07 확정, 2026-09-15 `네`/`그렇다` 추가):
  ...
  - **`그`/`저`/`뭐`/`뭔가`/`좀`/`막`/`그냥`/`같다`/`네`/`그렇다`** — 실제 사전 단어라 STT 텍스트에 항상 남는다(VAD 불필요). 다만 "그 사람이"(지시대명사)/"네, 알겠습니다"(실제 답변)처럼 필러가 아닌 정상 용법과 헷갈리기 쉬워서, 단순 문자열 매칭이 아니라 **LLM이 문맥을 보고 필러성 사용인지 판단**해서 채운다(정합성 판정과 같은 LLM 호출 단계에서 처리). `네`는 정확히 일치할 때만, `그렇다`는 "그렇습니다"/"그렇죠"/"그러네요" 등 어미 활용형(`같다`와 동일한 방식)까지 후보로 잡는다.
